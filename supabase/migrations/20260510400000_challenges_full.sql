...
-- Rename legacy `points` → keep as-is (unused by new code, default 0)
...
-- Any family member who is a participant can insert logs (via RPC only, but belt-and-suspenders)
...
-- Drop overly restrictive parent-only policies
...
-- Any family member can join a challenge in their family
...
-- Atomic: insert log + update participant value + check completion
...
begin
  ...
  if v_challenge.type = 'solo' then
    -- Solo: check if THIS participant hit the target
    if v_new_value >= v_challenge.target_value then
      -- Mark this participant as completed
      update public.challenge_participants
      set completed_at = now()
      where id = v_participant.id and completed_at is null;
      ...
      -- Award points to this participant (they hit their target)
    ...
    if v_total_value >= v_challenge.target_value then
      ...
      -- Award points to ALL participants who contributed
      perform public.award_points(
        cp.family_member_id,
        v_challenge.reward_xp,
        v_challenge.reward_coins,
  ...
  -- For collaborative/boss, include total
...
-- Allows any family member to join an active challenge
...
begin
  ...
  -- Insert (idempotent — unique constraint will prevent duplicates)
...
-- ── 10. Trigger: notify when someone logs progress ──
...
-- ── 12. Add challenge notification types to prefs default ──
-- Update the default to include new notification types
-- (existing rows keep their current prefs; new members get the updated default)
