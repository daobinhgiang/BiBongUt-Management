-- Add task_type enum and daily habits seeding RPC

-- 1. Create the enum
create type public.task_type as enum ('regular', 'daily_habit');

-- 2. Add column to tasks table
alter table public.tasks
  add column task_type public.task_type not null default 'regular';

-- 3. Partial index for fast filtering by task_type
create index idx_tasks_type_active
  on public.tasks (task_type)
  where is_active = true;

-- 4. Unique partial index to prevent duplicate daily habit seeding
create unique index uq_daily_habit_per_member_day
  on public.tasks (family_id, assignee_id, title, due_date)
  where task_type = 'daily_habit' and is_active = true;

-- 5. Update complete_task RPC to carry task_type and creator_tz into recurring insert
...
begin
  ...
  -- 5. If recurring, create next instance (now includes task_type + creator_tz)
...
-- 6. Create ensure_daily_habits RPC
...
declare
  v_today date;
begin
  -- Calculate today in the member's timezone
  v_today := (now() at time zone p_tz)::date;

  -- Insert daily habits with ON CONFLICT DO NOTHING (idempotent)
  insert into public.tasks (
    title, assignee_id, difficulty, points, coins_reward,
    due_date, recurrence, family_id, created_by, task_type, creator_tz
  ) values
    ('Eat breakfast', p_member_id, 'easy', 5, 2, v_today, 'daily', p_family_id, p_member_id, 'daily_habit', p_tz),
    ('Brush teeth', p_member_id, 'easy', 5, 2, v_today, 'daily', p_family_id, p_member_id, 'daily_habit', p_tz),
    ('Drink water', p_member_id, 'easy', 5, 2, v_today, 'daily', p_family_id, p_member_id, 'daily_habit', p_tz)
